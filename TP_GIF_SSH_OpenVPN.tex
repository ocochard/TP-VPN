...
La différence entre deux techniciens ce fait uniquement à la qualité du travail écrit rendu. La notation de votre rapport prendra donc en compte:
\begin{enumerate}
...
Les éditeurs de textes vim et nano ont aussi été installés.
...
\begin{enumerate}
\item Ajouter la ligne \shellcmd{sshd\_enable="YES"} dans le fichier /etc/rc.conf;
...
\begin{itemize}
\item \shellcmd{http://172.16.254.1}
\item \shellcmd{http://[fc00:dead:beef::1]}
\end{itemize}
...
\item Il vous a transmis une archive (succursale\_BIN\_cle\_ssh.tgz) contenant les clés SSH privée et publique de l'utilisateur ainsi que l'emprunte SSH de la clé du concentrateur VPN (dans le dossier /home/etudiant).
\end{enumerate}
...
\begin{enumerate}
\setcounter{enumi}{\value{saveenum}}
\item Quelle est la clé, demandée par la commande ssh, à utiliser pour chiffrer votre session à destination du siège ?
\item Comme il faut 2 clés pour chiffrer, quelle est la deuxième clé utilisée par SSH et comment la récupère-t-il?
...
\item Qu'est-ce qu'une attaque de type «~man in the middle~» ? Comment s'en protéger ? et comment SSH protège de ce type d'attague ?
\setcounter{saveenum}{\value{enumi}}
\end{enumerate}
}
...
\item Protéger vos clés pour que personne d'autre que votre utilisateur ne puisse y accéder par la commande \shellcmd{chmod 600 id\_rsa*}. Le client SSH vérifiera les droits d'accès et refusera d'utiliser une clé dont les permissions d'accès ne sont pas assez restrictives ;
\item Lancer la commande suivante (sur une seule ligne) : \shellcmd{ssh -N -L8080:172.16.254.1:80 -g~-i~/chemin/votre.clé.ssh~succursale\_BIN@2.2.2.254}
...
\shellcmd{http://172.16.BIN.254:8080} et/ou \shellcmd{http://[fc00:BIN::254]:8080}
\end{enumerate}
\cadre{\question{Questions}
\begin{enumerate}
\setcounter{enumi}{\value{saveenum}}
...
Pour le paramétrage du client VPN, vous commencerez par désarchiver vos certificats OpenVPN (transmis par le siège en même temps que les clés SSH).\\
\cadre{\question{Questions}
\begin{enumerate}
\setcounter{enumi}{\value{saveenum}}
\item Détaillez les fichiers présents dans l'archive succursale\_BIN\_cles\_openvpn.tgz et leurs rôles;
...
Cet exercice va vous demander de générer votre propre fichier ca.crt : Attention de ne  pas mélanger ce nouveau avec celui de l'exercice précédent car ils portent le même nom. De plus, lors de la copie scp, vérifiez que vous n'écrasez pas un fichier déjà existant sur la machine du binôme voisin.

Dans le répertoire /usr/local/share/easy-rsa/ se trouvent plusieurs scripts et fichiers de configurations qui vont vous aider à mettre en place l’architecture de gestion des clés nécessaires pour faire fonctionner OpenVPN.
...
Puis exécutez le script \shellcmd{easyrsa build-ca nopass} et examinez le contenu du répertoire keys/. 
...
Exécutez le script \shellcmd{easyrsa build-server-full concentrateur NOM-DE-VOTRE-ROUTEUR nopass} puis examinez le contenu du répertoire keys/.
...
Vous allez maintenant générer un certificat client par la commande \shellcmd{easyrsa build-client-full BINOME-?i nopass} avec ? correspondant au numéro du binôme voisin.
